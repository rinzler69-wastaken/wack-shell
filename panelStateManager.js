...
const DARK_PANEL_BLEND_ALPHA = 0.05;

export const PanelState = Object.freeze({ NONE: 'none', VIBRANCY: 'vibrancy', PROXIMITY: 'proximity' });
...
export default class PanelStateManager {
    constructor(extension, settings) {
        ...
        this._panelState = PanelState.NONE;
        this._panelStateDesired = PanelState.NONE;
    }
    ...
    get panelVisualsSuppressed() {
        return this._panelVisualsSuppressed;
    }

    get desiredPanelState() {
        return this._panelStateDesired;
    }

    setPanelState(state) {
        if (!Object.values(PanelState).includes(state))
            state = PanelState.NONE;

        this._panelStateDesired = state;

        // Suppression hides the current visual state but deliberately does not
        // erase the desired state. Overview can therefore change workspaces
        // while suppressed and simply update this value; resume paints the
        // latest destination state.
        if (this._panelVisualsSuppressed)
            return;

        if (state === this._panelState) {
            if (state === PanelState.PROXIMITY) {
                // Reasserting the same logical state is meaningful: the actor
                // may have been hidden by suppression or temporarily covered.
                this.setPanelProximity(true);
            } else if (state === PanelState.VIBRANCY) {
                this.applyVibrancyStyle();
            } else if (!this._panelProximityActive) {
                this._applyNoneState();
            }
            return;
        }
        ...
        if (state === PanelState.VIBRANCY) {
            if (this._panelProximityActive) {
                this.setPanelProximity(false, () => {
                    if (this._panelStateDesired === PanelState.VIBRANCY && !this._panelVisualsSuppressed)
                        this.applyVibrancyStyle();
                });
            } else {
                this.applyVibrancyStyle();
        ...
        if (this._panelProximityActive) {
            this.setPanelProximity(false, () => {
                if (this._panelStateDesired === PanelState.NONE && !this._panelVisualsSuppressed)
                    this._applyNoneState();
            });
        } else {
            this._applyNoneState();
    ...
    _applyNoneState() {
        ...
        for (const actor of [this._panelBackdrop, this._panelProximityOverlay]) {
        ...
        try {
            ...
            for (const cls of ['panel-ventura-light', 'panel-bigsur', 'panel-proximity', 'panel-proximity-light', 'panel-proximity-dark', 'light-contrast'])
    ...
    setPanelSuppressed(suppressed, onSettled = null) {
        if (suppressed) {
            if (this._panelVisualsSuppressed) {
            ...
            this._cancelProximityAnimation();
            const actors = [this._panelBackdrop, this._panelProximityOverlay];
            let remaining = 0;
            const done = () => {
                remaining--;
                if (remaining > 0 || token !== this._panelVisualSuppressionToken || !this._panelVisualsSuppressed)
                    return;
            ...
            for (const actor of actors) {
                if (actor && !actor.destroyed && actor.visible && actor.opacity > 0) {
                    remaining++;
                    actor.remove_all_transitions();
                    actor.ease({ opacity: 0, duration: 180, mode: Clutter.AnimationMode.EASE_OUT, onComplete: done });
                }
            }
            if (remaining === 0) {
                for (const actor of actors) {
                    if (actor && !actor.destroyed) { actor.set_opacity(0); actor.hide(); }
                }
                if (onSettled) onSettled();
            }
            return;
        ...
        this._panelVisualsSuppressed = false;
        this.setPanelState(this._panelStateDesired);
        if (onSettled) onSettled();
    }
    ...
    async _regeneratePanelStrip() {
        ...
        this._stripPromise = (async () => {
            try {
                ...
                const useVenturaBlend = effectiveStyle === 2 && !isDark;
                const blendColor = useVenturaBlend ? { r: 255, g: 255, b: 255 } : { r: 0, g: 0, b: 0 };
                const blendAlpha = useVenturaBlend ? this._getVenturaWhiteBlendAlpha() : DARK_PANEL_BLEND_ALPHA;
                const blendVariant = useVenturaBlend ? 'ventura' : 'standard';
                ...
                const cacheKey = `${targetUri}_${mtime}_${size}_${monitor.width}x${monitor.height}_${scale}_${pictureOptions}_${primaryColor}_${secondaryColor}_${shadingType}_${blurRadius}_${brightness}_${blendVariant}_${blendAlpha.toFixed(4)}_${stripWidth}x${stripHeight}_v11`;
                const hash = GLib.compute_checksum_for_string(GLib.ChecksumType.MD5, cacheKey, -1).substring(0, 8);
                const suffix = useVenturaBlend ? '-ventura' : '';
                const path = GLib.build_filenamev([PANEL_CACHE_DIR, `${PANEL_CACHE_PREFIX}${USER_NAME}-${hash}${suffix}.png`]);
                ...
                if (imagePath) {
                    ...
                    try {
                        ...
                        if (dir.query_exists(null)) {
                            ...
                            const currentSuffix = `${hash}${suffix}.png`;
                            let fileInfo;
                            while ((fileInfo = enumerator.next_file(null)) !== null) {
                                const name = fileInfo.get_name();
                                if (!name.startsWith(`${PANEL_CACHE_PREFIX}${USER_NAME}-`) || name.endsWith(currentSuffix))
                                    continue;
                                const isSameVariant = useVenturaBlend
                                    ? name.endsWith('-ventura.png')
                                    : !name.endsWith('-ventura.png');
                                if (isSameVariant) {
    ...
    _ensurePanelBackdrop() {
        ...
        Main.panel.insert_child_at_index(this._panelProximityOverlay, 1);
    ...
    _allocatePanelBackdrop() {
        ...
        if (proximityOverlay && !proximityOverlay.destroyed) {
            ...
            try {
                Main.panel.set_child_above_sibling(proximityOverlay, this._panelBackdrop);
    ...
    _applyProximityPanelClasses() {
        const isDark = this._isDarkColorScheme();
        Main.panel.add_style_class_name('panel-proximity');
        Main.panel.remove_style_class_name(isDark ? 'panel-proximity-light' : 'panel-proximity-dark');
        Main.panel.add_style_class_name(isDark ? 'panel-proximity-dark' : 'panel-proximity-light');
        Main.panel.remove_style_class_name('panel-ventura-light');
        Main.panel.remove_style_class_name('panel-bigsur');
        Main.panel.remove_style_class_name('light-contrast');
        this._settingStyle = true;
    ...
    _clearProximityPanelClasses() {
        for (const cls of ['panel-proximity', 'panel-proximity-light', 'panel-proximity-dark'])
            Main.panel.remove_style_class_name(cls);
    }

    updatePanelProximityColor(css) {
        this._setProximityPanelColor(css);
        if (this._panelState === PanelState.PROXIMITY || this._panelStateDesired === PanelState.PROXIMITY)
            this._applyProximityPanelClasses();
    }
    ...
    setPanelProximity(active, onSettled = null) {
        ...
        if (active) {
            ...
            if (wasActive) {
                // Proximity can remain logically active while its visual actor
                // has been hidden by Overview/lockscreen suppression.  In that
                // case this is not a redundant observation: it is a visual
                // restoration and must bring the P surface back.  v30 did this
                // implicitly by restoring the actor on every P reassertion.
                const proximityVisuallyMissing = !overlay || overlay.destroyed
                    ? false
                    : (!overlay.visible || overlay.opacity <= 0);

                if (proximityVisuallyMissing) {
                    if (backdrop && !backdrop.destroyed) {
        ...
        const shouldRestoreVibrancy = this._panelStateDesired === PanelState.VIBRANCY &&
            this.vibrancyActive &&
        ...
        this._animatePanelVisualLayers(shouldRestoreVibrancy ? 255 : 0, 0, () => {
            if (shouldRestoreVibrancy)
                ...
                this._applyNoneState();
            this._clearProximityPanelClasses();
            if (onSettled) onSettled();
        });
    }
    ...
    applyVibrancyStyle() {
        ...
        // Vibrancy is a state, not an independent subsystem. Async wallpaper
        // regeneration and panel style notifications may call this method at
        // any time, but they must never paint V over P or NONE.
        if (this._panelStateDesired !== PanelState.VIBRANCY || this._panelState === PanelState.PROXIMITY)
            return;
        ...
        // The static wallpaper already contains the resolved Ventura/standard
        // blend. Keep Main.panel transparent so the processed snapshot is the
        // only panel background surface.
        this._ensurePanelBackdrop();
